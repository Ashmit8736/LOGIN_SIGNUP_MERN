...
const Tasks = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const res = await axios.get('https://login-signup-model-3.onrender.com/api/task', {
  ...
  const deleteTask = async (id) => {
    if (window.confirm("Are You Sure want to delete this task?")) {
      try {
        ...
        await axios.delete(`https://login-signup-model-3.onrender.com/api/task/${id}`, {
         headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('https://login-signup-model-3.onrender.com/api/task', formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
